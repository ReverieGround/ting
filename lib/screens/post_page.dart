...
class _PostPageState extends State<PostPage> {
  ...
  Future<void> postComment() async {
    ...
    String postId = widget.post['post_id']; // ✅ 포스트 ID 가져오기
  ...
  Widget build(BuildContext context) {
    List<dynamic> imageUrls = widget.post['image_urls'] as List<dynamic>? ?? [];
  ...
  Widget _buildImagePannel(imageUrls, height, width) {
    bool hasMultipleImages = imageUrls.length > 1;
    String thumbnailUrl = imageUrls.isNotEmpty ? imageUrls[0] : 'https://via.placeholder.com/600';
    return Stack(
      children: [
        ClipRRect(
  ...
  Widget _buildCommentPannel(){
    return Expanded(
      child: Container(
        padding: EdgeInsets.all(12),
        decoration: BoxDecoration(color:Colors.white),
        child: PostComments(postId: widget.post['post_id'], refreshCallback: refreshComments),
      ),
    );
  }
