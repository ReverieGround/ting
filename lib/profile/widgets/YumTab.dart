...
class _YumGridState extends State<YumTab> {
  ...
  Map<String, List<dynamic>> groupPostsByDay(List<dynamic> posts) {
    ...
    for (var post in posts) {
      final hasImages = post.imageUrls is List && (post.imageUrls as List).isNotEmpty;
      if (hasImages) {
        final rawCreatedAt = post.createdAt;
  ...
  Widget buildGalleryCard(List<dynamic> posts) {
    ...
    return  GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final imageUrls = post.imageUrls; // 너의 post 모델에 따라 맞게 수정
